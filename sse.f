...
*
      integer kw,j,k,i
*
      real*8 mass,mt,z,zpars(20),mzams
...
C *
C       j = 0
C       if(scm(1,1).lt.0.0) goto 50
C *
C * The scm array stores the stellar parameters at the specified output
C * times. The parameters are (in order of storage):
C *
C *    Time, stellar type, initial mass, current mass, log10(L), log10(r),
C *    log10(Teff), core mass, epoch and spin.
C *
C       OPEN(23,file='evolve.dat',status='unknown')
C       text1 = ' Tev(Myr)    type      Mo        Mt      log10(L) '
C       text2 = ' log10(R) log10(Teff)  Mc        Menv     '
C       text3 = ' epoch      spin'
C       WRITE(23,'(a,a,a)')text1,text2,text3
C  30   j = j + 1
C       if(scm(j,1).lt.0.0)then
C          scm(j-1,1) = scm(j,1)
C          j = j - 1
C       endif
C       WRITE(23,99)(scm(j,k),k=1,8),scm(j,10),scm(j,12),scm(j,13)
C       if(scm(j,1).ge.0.0) goto 30
C       CLOSE(23)
C  99   FORMAT(8f10.4,1p,e12.4,0p,f12.4,1p,e12.4)
C *
C * The spp array acts as a log, storing the time and mass at each change
C * of evolution stage.
C *
C       j = 0
C  50   j = j + 1
C       if(spp(j,1).lt.0.0) goto 60
C       kw = INT(spp(j,2))
C       WRITE(*,100)label(kw+1),spp(j,1),spp(j,3)
C       goto 50
C  60   continue
C  100  format(a30,' Time ',f10.1,' Mass ',f7.3)
C       WRITE(*,*)

  100  format(f4.2, f8.3, f11.2, I5, f7.2)
      
      DO j=2,50000
          if (INT(scm(j-1,2)) .lt. 10 .and. INT(scm(j,2)) .gt. 10) then
              kw = INT(scm(j,2))
              WRITE(*,100) mzams,scm(j,4),scm(j,1),int(scm(j,2)),10.d0**scm(1,6)
              exit
          endif
      ENDDO
      ENDDO
